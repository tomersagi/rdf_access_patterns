...
public class StatsGenerator {
    ...
    private void prepareStats(Model graphModel, QuerySolution classTypeTriple) {
        ...
        RdfUtils.runAQuery(countPropClassQuery, graphModel).forEachRemaining(countPropTriple -> {
            ...
            RdfUtils.runAQuery(getMaxCountQuery(classTypeTriple, countPropTriple), graphModel).forEachRemaining(val -> {
                maxCountPropResult.set(val.get("maxCount").asLiteral().getInt() + "\"" + "^^xsd:integer");
            });


            RdfUtils.runAQuery(getMinCountQuery(classTypeTriple, countPropTriple), graphModel).forEachRemaining(val -> {
            ...
            RdfUtils.runAQuery(getDistinctPropertyCountQuery(classTypeTriple, countPropTriple), graphModel).forEachRemaining(dct -> {
    ...
    private String getMaxCountQuery(QuerySolution classTypeTriple, QuerySolution countPropTriple) {
        return "SELECT (MAX(?Y) AS ?maxCount) WHERE {\n" +
